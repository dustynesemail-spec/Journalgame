...
const enemyBook={
 Scratchling:{name:'Scratchling',icon:'🐾',hp:14,damage:1,skill:'Double Scratch',story:'A scrappy minion of Evil Kitty. Two quick scratches strike the announced survivor.'},
 Gazer:{name:'Little Gazer',icon:'🧿',hp:15,damage:1,skill:'Leeching Glance',story:'A loose eye from the Wall of Eyes. It hurts a survivor and restores 2 of its own Hearts.'},
 Mite:{name:'Watcher Mite',icon:'🕷️',hp:14,damage:1,skill:'Withering Whisper',story:'A tiny scout of the One Who Watches. Its target’s next attack deals 1 less damage.'},
 SandProwler:{name:'Sand Prowler',icon:'🐈‍⬛',hp:15,damage:2,skill:'Sand Pounce',story:'Evil Kitty’s desert scout ignores 1 point of Guard when it pounces.'},
 FrostWisp:{name:'Frost Wisp',icon:'❄️',hp:14,damage:1,skill:'Ice Shell',story:'A frozen servant of the Wall of Eyes. After attacking, it shields itself against 2 damage.'},
 EvilKitty:{name:'Evil Kitty',icon:'🐈',hp:24,damage:2,boss:true,skill:'Royal Clawstorm',story:'The first portal guardian sends her minions across the island. She strikes twice, for 2 damage each.'},
 Eyes:{name:'Wall of Eyes',icon:'👁',hp:28,damage:1,boss:true,skill:'All-Seeing Drain',story:'The second portal guardian attacks every standing survivor and restores 2 of its own Hearts.'},
 Watcher:{name:'The One Who Watches',icon:'👁️',hp:32,damage:2,boss:true,skill:'Unmaking',story:'The final portal guardian removes its target’s Guard before striking for 2 damage.'}
};
...
function validCombatSave(s){
 ...
 if(s.journey!==undefined&&(!s.journey||!num(s.journey.quietTurns,1)||!num(s.journey.searches)||!Array.isArray(s.journey.bosses)||s.journey.bosses.length>3||!s.journey.bosses.every((key,i)=>key===portalBosses[i])))return false;
...
function checkEncounter(){
 ...
 const encounter=journey.quietTurns>=1||Math.random()<.65;
 journey.quietTurns=encounter?0:1;queueSave();
...
function startBattle(enemy){
 ...
 battle={enemy,phase:'intro',hp,maxHp:hp,round:1,turn:order.find(i=>!players[i].ko),order,acted:[false,false,false,false],guard:[0,0,0,0],marked:false,intent:0,tick:0,log:[],weakness:[0,0,0,0],enemyGuard:0,burn:0,smoke:0,sinceEnemy:0,nextTurn:null,lastMove:''};
...
function showBattle(){
 ...
 if(b.phase==='won'){
  showModal('<div class="battleEnemy" aria-hidden="true">✨</div><h2>TEAM VICTORY!</h2><p>'+e.name+' is defeated. Everyone’s map position and journey actions are preserved.</p><p><b>Loot: 1 Gold, 2 Food, 1 Healing Kit, and 3 '+weaponBook[b.order[0]].ammo+'.</b></p><p>'+(e.boss?'Guardian defeated! The portal restores 2 Hearts to everyone, adds 2 more kits, and 4 of every ammo.':'A useful regional item is also in your pack.')+'</p><div class="battleTeam">'+players.map((_,j)=>battleHero(j)).join('')+'</div><button class="choice" id="leaveBattle">CONTINUE JOURNEY</button>',false);
  $('leaveBattle').addEventListener('click',finishBattle);return;
 }
 const header='<div class="battleHeading"><span class="battleEnemy" aria-hidden="true">'+e.icon+'</span><div><h2>'+e.name+'</h2><b>♥ '+b.hp+' / '+b.maxHp+'</b><p>Battle round '+b.round+' · Next hit: '+players[b.intent].role+' · '+e.skill+'</p><small>'+e.story+'</small><p>Enemy Guard '+b.enemyGuard+' · Burn '+b.burn+' · Smoke '+b.smoke+'</p></div></div>';
...
function awardBattleVictory(){
 if(!battle||battle.phase==='won')return;
 const b=battle,e=enemyBook[b.enemy];b.phase='won';b.hp=0;battlesWon++;res.Gold++;res.Food+=2;inventory['Healing Kit']++;gear.ammo[weaponBook[b.order[0]].ammo]+=3;
 if(e.boss){
  ...
  const loot=regionalLoot[players[active].loc]||regionalLoot.Beach;const useful=loot.filter(x=>itemBook[x.name]);const item=useful[Math.floor(Math.random()*useful.length)];awardSupply(item.name,item.n);
 }
 if(Object.hasOwn(monsters,b.enemy))monsters[b.enemy]=null;discoveries.add('Defeated '+e.name);
}
function enemyRespond(){
 const b=battle,e=enemyBook[b.enemy];b.sinceEnemy=0;const report=[];
